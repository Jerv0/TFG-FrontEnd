...
const userTareas = async (id: string) => {
    try {
        ...
        idUserNoActivo = allUsers.filter((element): element is string => typeof element === 'string' && !idUserActivo.includes(element)).concat(idUserActivo.filter((element) => !allUsers.includes(element)));
...
.route {
    /** Quita el marcado por ser un enlace */
    color: inherit;
