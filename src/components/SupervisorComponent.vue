...
const filtroPacientes = computed(() => {
    return pacientesUsuarios.value.filter((paciente) => paciente[filtroSeleccionado.value as keyof PacienteUsuario].toLowerCase().includes(busqueda.value.toLowerCase()));
});
...
<template>
    <q-layout view="hHh lpR fFf">
        <q-page-container>
            ...
            <!-- Barra de búsqueda -->
            <q-input v-model="busqueda" class="q-mt-md q-mb-md q-mtbuscador" placeholder="Buscar paciente..." filled />
            ...
            <div class="q-gutter-md q-px-md">
                <q-option-group
                    ...
                    :options="[
                        ...
                        { label: 'Especialidad', value: 'especialidad_requerida' },
                    ]"
                    color="primary"
                />
            </div>
            ...
            <div v-if="filtroPacientes.length > 0" class="q-pa-md">
                <q-card v-for="el in filtroPacientes" :key="el.id" class="q-mb-md">
                    <q-card-section>
                        <div class="patient-header" @click="changeView(parseInt(el.id))">
                            <div class="text-h6">{{ el.nombre }} {{ el.apellido }}</div>
                            <q-icon :name="infoPaciente === (el.id ? parseInt(el.id) : null) ? 'expand_less' : 'expand_more'" />
                        </div>
                        ...
                        <q-collapse-transition>
                            <div v-if="infoPaciente === (el.id ? parseInt(el.id) : null)" class="patient-info">
                                <q-tabs v-model="activeTab" class="text-teal">
                                    <q-tab name="userInfo" label="Información del Usuario" />
                                    <q-tab name="medicalInfo" label="Información Médica" />
                                </q-tabs>
                                ...
                                <q-btn icon="add" @click="userIdActivo(el.id)" to="/creacionTareas" label="Agregar Tarea" color="primary" class="q-mt-md" />
                            </div>
                        </q-collapse-transition>
                    </q-card-section>
                </q-card>
            ...
            <div v-else>
                <q-banner class="q-mt-md q-mb-md">No se encontraron resultados.</q-banner>
            </div>
        </q-page-container>
    </q-layout>
</template>
